...
- **다국어 지원**: UI·메시지·md의 역할 헤딩이 브라우저 언어를 따라갑니다 (영어 기본, 한국어 지원 — `_locales/`에 언어 추가 가능).
...
> **Obsidian 사용자 권장 설정 (필수 전제)**
> Chrome의 기본 다운로드 위치를 **Obsidian Vault 루트 폴더**로 설정하세요. 그래야 저장 즉시 노트와 첨부가 Vault 안에 생성되어 복사·이동 과정이 아예 사라집니다.
> 다운로드 위치가 기본(`~/Downloads`)인 채로 Obsidian 모드를 쓰면 `Downloads/` 안에 노트·첨부 폴더가 생겨서, Vault로 **매번 두 번 옮겨야** 합니다 — 그럴 바엔 기본 모드가 낫습니다.
