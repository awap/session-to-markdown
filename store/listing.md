...
## 버전 업데이트 방법 (이미 제출/게시된 항목에 새 패키지 올리기)

- 심사 대기 중에는 새 패키지를 못 올린다. 항목 상태에 있는 **"검토 취소"**를 누르거나, 게시될 때까지 기다린다.
- 그 후 대시보드 → 해당 항목 → 왼쪽 메뉴 **"패키지(Package)"** 탭 → **"새 패키지 업로드"** → 새 zip 업로드 → 다시 "검토를 위해 제출".
- 새 패키지의 manifest 버전은 반드시 기존보다 높아야 한다 (예: 0.2.2 → 0.3.0).
- 2026-07-03 첫 제출: 이전 버전 패키지로, 배포 지역 한국 한정. 심사 통과 후 0.3.0(다국어) 올리면서 지역 확장 예정.
